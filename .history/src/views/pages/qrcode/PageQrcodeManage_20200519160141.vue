...
<script>
  import { changeTitle, editChildQrcode, requestQrcode } from '@/api/qrcode'
        ...
        requestQrcode(params).then(res => {
          if (res.code === 200) {
            ...
            this.tableData.map(v => {
              console.log(new Date(v.create_time * 1000))
              let aa = new Date(v.create_time * 1000)
              console.log(aa.getFullYear() + " " + aa.getMonth() + " "aa.getSeconds)
              v.create_time = new Date(v.create_time * 1000)
              v.update_time = new Date(v.update_time * 1000)
            })
          }
        }).catch(err => {
          console.log(err)
