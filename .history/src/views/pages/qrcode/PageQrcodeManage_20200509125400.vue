<template>
  <div>
    <el-card style="width: 1200px;" v-if="show===1">
    ...
    <el-card style="width: 1200px;" v-if="show===2">
      <div slot="header">
        <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">新增子码</el-button>
        <el-button size="mini" @click="handleDelete(scope.$index, scope.row)">返回上一页</el-button>
      </div>
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column label="操作" fixed="right" width="300">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleEditChild(scope.$index, scope.row)">编辑</el-button>
    ...
    <el-card style="width: 1200px;" v-if="show===3">
  ...
  export default {
    ...
    methods: {
      ...
      handleEditChild(index, row) {
        this.childData = this.tableData[index]
        this.show = 2
      },
