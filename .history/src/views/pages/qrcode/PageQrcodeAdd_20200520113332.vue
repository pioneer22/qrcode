...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      uploadFile(file) {
        if (this.formDate.has('file')) {
          let fileArray = this.formDate.get('file')
          console.log("fileArray:", this.formDate.get('file'))
          fileArray.push(file.file)

          this.formDate.set('file', fileArray)
        } else {
          let fileArray1 = []
          fileArray1.push(file.file)
          this.formDate.append('file', fileArray1);
        }
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.$refs.upload.uploadFiles.length === 0) {
              ...
              this.formDate.append('title', this.ruleForm.title);
              this.formDate.append('num', this.ruleForm.num);
              this.formDate.append('validity', this.ruleForm.validity);

              uploadQrcode(this.formDate).then(res => {
                if (res.code === 200) {
                  this.$message({
                    type: 'success',
                    message: '新建活码成功~'
              ...
              this.$axios.post(process.env.VUE_APP_URL + '/qrcode/index/create_qrcode', this.formDate, config).then(res => {
                if (res.code === 200) {
                  this.$message({
                    type: 'success',
                    message: '新建活码成功~'
