  ...
  <div>
    <el-card class="card" v-if="show===1" shadow="hover">
      ...
      <el-table :data="tableData" :default-sort="{prop: 'id',order: 'ascending'}" style="width: 100%">
        ...
        <el-table-column label="活码标题">
          <template slot-scope="scope">
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column label="今日扫码次数">
          <template slot-scope="scope">
            <span>{{ scope.row.dayNum || '未知' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="总扫码次数">
          <template slot-scope="scope">
    ...
    <el-card class="card" v-if="show===2" shadow="hover">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column label="排序" sortable prop="sortid">
          <template slot-scope="scope">
        ...
        <el-table-column label="子码标题">
          <template slot-scope="scope">
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column label="识别量">
          <template slot-scope="scope">
            <span>{{ scope.row.dayNum || '未知' }}</span>
          </template>
        </el-table-column>

        <el-table-column label="子码上传日期">
          <template slot-scope="scope">
            <span>{{ scope.row.date }}</span>
          </template>
        </el-table-column>
      ...
      <el-form :model="childData" :rules="rules" ref="childData" label-width="100px">
        ...
        <div style="padding-left: 20px;">
          <el-upload ref="upload" class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/"
            :show-file-list="false" :on-success="handleSuccess" :on-change="handleChange" :data="childData" :limit="1"
