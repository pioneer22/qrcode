            ...
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
              ...
              <div class="upload-content">
                <el-upload class="upload-demo" ref="upload" :auto-upload="false" drag :action="url" list-type="picture"
                  :on-success="handleSuccess" :data="fileList" :before-upload="beforeUpload" multiple>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 生成活码
      submitForm(formName) {
        this.fileList = { "file": this.$refs.upload.uploadFiles }
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.fileList.file.length === 0) {
              ...
              uploadQrcode(this.fileList).then(res => {
                console.log("res:", res)
              }).catch(err => {
                console.log(err)
              })
